...
{
    std::string ptx = get_file_contents("ptx/raymarch.cu.ptx");

    std::string shade_hook_src = ""
    ...
    "__device__ float3 shade_hook("
            ...
            ")"
    "{\n"
"        return make_float3(1.0f, 0.0f, 0.0f) + make_float3(1.0f, 0.0f, 0.0f);\n"
    "}\n } \n";

    RuntimeCompiler program("shade_hook", shade_hook_src);

    std::string result( program.getResult() );

    // Delete "Generated by.." comments and version info (assume it's the same target/version)
    for(int i =0 ; i < 13; i++)
    {
        result.erase(0, result.find('\n') + 1);
    }

    std::istringstream f(ptx);
    std::string line;

    std::vector<std::string> lines;

    unsigned int startExtern;
    unsigned int endExtern;
    {
        ...
        while (std::getline(f, line))
        {
            if( !foundExtern )
            {
                ...
                size_t shade_hookPos = line.find("shade_hook");
    ...
    // Remove old
    lines.erase(lines.begin() + startExtern, lines.begin() + endExtern + 1);

    std::vector<std::string> src_lines;
    std::istringstream src_stream(result);
    std::string src_line;
    ...
    lines.insert(lines.begin() + startExtern, src_lines.begin(), src_lines.end());
    ...
    std::string concatptx = boost::algorithm::join(lines, "\n");
