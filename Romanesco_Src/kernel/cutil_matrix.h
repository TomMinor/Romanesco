 ...
 * Modified from the Optix Matrix library to work with NVRTC by Tom Minor, 2016
 */

#ifndef CUTIL_MATRIX_H
...
//namespace optix {
  ...
  template <unsigned int M, unsigned int N> class Matrix;
   ...
   template <unsigned int M> OPTIXU_INLINE __device__ Matrix<M,M>& operator*=(Matrix<M,M>& m1, const Matrix<M,M>& m2);
   RT_MAT_DECL OPTIXU_INLINE __device__ Matrix<M,N>& operator-=(Matrix<M,N>& m1, const Matrix<M,N>& m2);
   RT_MAT_DECL OPTIXU_INLINE __device__ Matrix<M,N>& operator+=(Matrix<M,N>& m1, const Matrix<M,N>& m2);
   RT_MAT_DECL OPTIXU_INLINE __device__ Matrix<M,N>& operator*=(Matrix<M,N>& m1, float f);
   RT_MAT_DECL OPTIXU_INLINE __device__ Matrix<M,N>&operator/=(Matrix<M,N>& m1, float f);
   RT_MAT_DECL OPTIXU_INLINE __device__ Matrix<M,N> operator-(const Matrix<M,N>& m1, const Matrix<M,N>& m2);
   RT_MAT_DECL OPTIXU_INLINE __device__ Matrix<M,N> operator+(const Matrix<M,N>& m1, const Matrix<M,N>& m2);
   RT_MAT_DECL OPTIXU_INLINE __device__ Matrix<M,N> operator/(const Matrix<M,N>& m, float f);
   RT_MAT_DECL OPTIXU_INLINE __device__ Matrix<M,N> operator*(const Matrix<M,N>& m, float f);
   RT_MAT_DECL OPTIXU_INLINE __device__ Matrix<M,N> operator*(float f, const Matrix<M,N>& m);
   RT_MAT_DECL OPTIXU_INLINE __device__ typename Matrix<M,N>::floatM operator*(const Matrix<M,N>& m, const typename Matrix<M,N>::floatN& v );
   RT_MAT_DECL OPTIXU_INLINE __device__ typename Matrix<M,N>::floatN operator*(const typename Matrix<M,N>::floatM& v, const Matrix<M,N>& m);
   template<unsigned int M, unsigned int N, unsigned int R> OPTIXU_INLINE __device__ Matrix<M,R> operator*(const Matrix<M,N>& m1, const Matrix<N,R>& m2);


  // Partial specializations to make matrix vector multiplication more efficient
  template <unsigned int N>
  OPTIXU_INLINE __device__ float2 operator*(const Matrix<2,N>& m, const typename Matrix<2,N>::floatN& vec );
  template <unsigned int N>
  OPTIXU_INLINE __device__ float3 operator*(const Matrix<3,N>& m, const typename Matrix<3,N>::floatN& vec );
  template <unsigned int N>
  OPTIXU_INLINE __device__ float4 operator*(const Matrix<4,N>& m, const typename Matrix<4,N>::floatN& vec );
  OPTIXU_INLINE __device__ float4 operator*(const Matrix<4,4>& m, const float4& vec );
  ...
  */
  ...
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ Matrix<M,N>::Matrix()
  {
  }

  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ Matrix<M,N>::Matrix( const Matrix<M,N>& m )
  {
  ...
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ Matrix<M,N>&  Matrix<M,N>::operator=( const Matrix& b )
  {
    for(unsigned int i = 0; i < M*N; ++i)
      m_data[i] = b[i];
    return *this;
  ...
  /*
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ float Matrix<M,N>::operator[]( unsigned int i )const
  {
  assert( i < M*N );
  return m_data[i];
  }
  ...
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ float& Matrix<M,N>::operator[]( unsigned int i )
  {
  return m_data[i];
  }
  */
  ...
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ typename Matrix<M,N>::floatN Matrix<M,N>::getRow( unsigned int m )const
  {
  ...
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ typename Matrix<M,N>::floatM Matrix<M,N>::getCol( unsigned int n )const
  {
  ...
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ float* Matrix<M,N>::getData()
  {
    return m_data;
  }
  ...
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ const float* Matrix<M,N>::getData() const
  {
    return m_data;
  }
  ...
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ void Matrix<M,N>::setRow( unsigned int m, const typename Matrix<M,N>::floatN &r )
  {
  ...
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ void Matrix<M,N>::setCol( unsigned int n, const typename Matrix<M,N>::floatM &c )
  {
  ...
  // Returns the transpose of the matrix.
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ Matrix<N,M> Matrix<M,N>::transpose() const
  {
    Matrix<N,M> ret;
  ...
  // Returns the determinant of the matrix.
  template<>
  OPTIXU_INLINE __device__ float Matrix<3,3>::det() const
  {
  ...
  // Returns the determinant of the matrix.
  template<>
  OPTIXU_INLINE __device__ float Matrix<4,4>::det() const
  {
    const float* m   = m_data;
    float d =
      m[0]*m[5]*m[10]*m[15]-
  ...
  // Returns the inverse of the matrix.
  template<>
  OPTIXU_INLINE __device__ Matrix<4,4> Matrix<4,4>::inverse() const
  {
    Matrix<4,4> dst;
    const float* m   = m_data;
  ...
  template<>
  OPTIXU_INLINE __device__ Matrix<4,4> Matrix<4,4>::rotate(const float radians, const float3& axis)
  {
  ...
  // This is a static member.
  template<>
  OPTIXU_INLINE __device__ Matrix<4,4> Matrix<4,4>::translate(const float3& vec)
  {
    Matrix<4,4> Mat = Matrix<4,4>::identity();
    float *m = Mat.getData();
  ...
  // This is a static member.
  template<>
  OPTIXU_INLINE __device__ Matrix<4,4> Matrix<4,4>::scale(const float3& vec)
  {
    Matrix<4,4> Mat = Matrix<4,4>::identity();
    float *m = Mat.getData();
  ...
  // This is a static member.
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ Matrix<N,N> Matrix<M,N>::identity()
  {
    float temp[N*N];
  ...
  // Ordered comparison operator so that the matrix can be used in an STL container.
  template<unsigned int M, unsigned int N>
  OPTIXU_INLINE __device__ bool Matrix<M,N>::operator<( const Matrix<M, N>& rhs ) const
  {
  ...
  OPTIXU_INLINE __device__ Matrix<3,3> make_matrix3x3(const Matrix<4,4> &matrix)
  {
    Matrix<3,3> Mat;
    float *m = Mat.getData();
